...
public:
  ...
  QTranslator *getTranslation() {
    ...
    if (!translator->load(QString("qfaktury_") + lang))
      auto x = translator->load(QString("qfaktury_") + lang, appPath + "/translations");
  ...
  QString getVersion(QString appName) {
    QString str = appName;
    str.truncate(2);
    return str.toUpper() + appName.right(6) + QString(" - wersja ") +
  ...
  int getCustomerType(QString custType) {
    if (custType.compare(QString("Firma")) == 0 ||
        custType.compare(QString("firma")) == 0) {
      return 0;
    } else if (custType.compare(QString("Urząd")) == 0 ||
               custType.compare(QString("urząd")) == 0) {
      return 1;
    } else {
      return 2;
    }
  ...
  int getProductType(QString prodName) {
    if (prodName.compare(QString("Towar")) == 0 ||
        prodName.compare(QString("towar")) == 0) {
      return 0;
    } else {
      return 1;
    }
  ...
  QString getCompanyNameTr() { return QString("Firma"); }

  QString getOfficeNameTr() { return QString("Urząd"); }
  ...
  QString getNameWithData(QString in) { return in + QString("DATA"); }
  ...
  QString getTPointStr() {
    QString tPoint = locale->groupSeparator();
