...
void Correction::correctionInit(bool mode) {
  ...
  labelReason1->setText(QString("Przyczyna korekty:"));
  ...
  textLabelSum1->setText(QString("Wartość korekty:"));
  textLabelSum2->setText(QString("Wartość faktury:"));
  textLabelSum3->setText(QString("Do zapłaty:"));

  invoiceType = QString("Korekta");
...
bool Correction::saveInvoice() {
  ...
  buyer.setAttribute(
      "tic", kht[3].replace(" ", "").replace(QString("NIP:"), ""));
  ret += kht[3].replace(" ", "").replace(QString("NIP:"), "");
  buyer.setAttribute("account", kht[4].replace(" ", "").replace(
                                    QString("Konto:"), ""));
  buyer.setAttribute(
      "phone", kht[5].replace(" ", "").replace(QString("Tel:"), ""));
  buyer.setAttribute(
      "email", kht[6].replace(" ", "").replace(QString("Email:"), ""));

  QString imprWeb =
      kht[7].replace(" ", "").replace(QString("Strona:"), "");
...
void Correction::readCorrData(QString invFile) {
  ...
  setWindowTitle(QString("Edytuje korektę"));
  ...
  if (!file.open(QIODevice::ReadOnly) || !doc.setContent(stream.readAll())) {
    ...
    if (check_file.exists() && check_file.isFile()) {
      ...
      QMessageBox::warning(
          this, QString("Brak dostępu"),
          QString("Plik przechowujący dane o fakturze korygującej w ścieżce ") +
              file.fileName() + QString(" nie istnieje.") +
              QString(" Jesteś pewien, że plik o tej nazwie nie jest "
  ...
  buyerName->setText(buyer.attribute("name") + "," + buyer.attribute("city") +
                     "," + buyer.attribute("street") + "," + QString("NIP: ") +
                     buyer.attribute("tic") + ", " + QString("Konto: ") +
                     buyer.attribute("account") + ", " + QString("Tel: ") +
                     buyer.attribute("phone") + ", " + QString("Email: ") +
                     buyer.attribute("email") + ", " + QString("Strona: ") +
...
void Correction::setIsEditAllowed(bool isAllowed) {
  ...
  if (isAllowed && (paysCombo->currentText() != QString("zaliczka")))
...
void Correction::schemaCalcSum() {
  ...
  if (diffTotal < 0)
    textLabelSum3->setText(QString("Do zwrotu:"));
  else if (diffTotal > 0)
    textLabelSum3->setText(QString("Do zapłaty:"));
  else
...
void Correction::makeInvoceProductsTitle(short a) {
  ...
  if (a == 1) {
    invStrList += QString("Pozycje na fakturze po korekcie:");
  }

  if (a == 0) {
    invStrList += QString("Pozycje na fakturze przed korektą:");
  }
...
void Correction::makeBeforeCorrSumm() {
  ...
  invStrList += "<td width=\"11%\" align=\"center\">" + QString("Kwota VAT") +
                "</td>"; // vat
  invStrList += "<td width=\"11%\" align=\"center\">" +
                QString("Wartość Brutto") + "</td>"; // gross
  invStrList += "</tr>";
  invStrList += "<tr class=\"productsSum\" valign=\"middle\">";
  invStrList += "<td align=\"right\">" + QString("Razem:") + "</td>";
  invStrList += "<td align=\"center\">" +
...
void Correction::makeInvoiceSummAll() {
  ...
  invStrList += QString("Wartość faktury: ") +
                sett().numberToString(origGrossTotal) + "<br/>";
  invStrList +=
      QString("Wartość korekty: ") + sett().numberToString(grossTotal) + "<br/>";

  if (diffTotal > 0) {

    invStrList += QString("Do zapłaty: ");
    invStrList += sum3->text() + " " + currCombo->currentText() + "<br/>";

  } else if (diffTotal < 0) {

    invStrList += QString("Do zwrotu: ");
    invStrList +=
  ...
  invStrList +=
      QString("Słownie: ") + sumCorrected + "<br/>";

  delete conv;

  if (paysCombo->currentText() == QString("gotówka")) {

    invStrList +=
        QString("Sposób płatności: ") + paysCombo->currentText() + "<br/>";

  } else if (paysCombo->currentText() == QString("zaliczka")) {
  ...
  } else if (paysCombo->currentText() == QString("przelew")) {

    invStrList +=
        QString("Sposób płatności: ") + paysCombo->currentText() + "<br/>";
    invStrList += "<span class=\"toPay\">";
    invStrList += QString("Termin płatności: ") +
  ...
  invStrList +=
      QString("Przyczyna korekty: ") + reasonCombo->currentText() + "<br/>";
  invStrList +=
  ...
  invStrList += QString("Ogółem stawkami:");
