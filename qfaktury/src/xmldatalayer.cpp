...
QVector<BuyerData> XmlDataLayer::buyersSelectAllData() {
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    ...
    for (QDomNode n = natur_person.firstChild(); !n.isNull();
      ...
      BuyerData buyerData;
      buyersElemToData(buyerData, n.toElement());
      buyerData.type = QString("Osoba fizyczna");
      buyerVec.push_front(buyerData);
    }
  }
...
InvoiceData XmlDataLayer::invoiceSelectData(QString name, int type,
  ...
  o_invData.customer =
      ...
      purchaser.attribute("street") + "," + QString("NIP: ") +
      purchaser.attribute("tic") + ", " + QString("Konto: ") +
      purchaser.attribute("account") + ", " + QString("Tel: ") +
      purchaser.attribute("phone") + ", " + QString("Email: ") +
      purchaser.attribute("email") + ", " + QString("Strona: ") +
  ...
  if (o_invData.paymentType == QString("zaliczka")) {
...
bool XmlDataLayer::invoiceInsertData(InvoiceData &oi_invData, int type) {
  ...
  if (oi_invData.paymentType == QString("zaliczka")) {
...
void XmlDataLayer::separateOldDocuments(QString path) {
  ...
  for (constIterator = yearsList.constBegin();
    ...
    QMultiHash<QString, QString>::const_iterator i = categorizedFiles.constBegin();
