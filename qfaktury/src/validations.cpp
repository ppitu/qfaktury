...
bool Validations::validateAccount(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany rachunek bankowy. Wymagany format to "
...
bool Validations::validateIDCard(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany numer dowodu osobistego. Wymagany format dowodu to "
...
bool Validations::validateNIP(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany numer NIP. Wymagany format NIP to XXX-XXX-XX-XX lub "
...
bool Validations::validatePass(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany numer seryjny paszportu. Wymagany format dowodu to "
...
bool Validations::validatePESEL(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany PESEL. Wymagany format to "
...
bool Validations::validatePkwiu(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany numer PKWiU. Wymagany format to XX(.XX.XX.X) , "
...
bool Validations::validateRegon(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(0, "QFaktury",
                         QString("Źle podany REGON. Wymagany format to "
...
bool Validations::validateTel(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany numer telefonu. Wymagany format to +XX(X od 5 do "
...
bool Validations::validateWebsite(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Źle podany adres internetowy. Format pozwala na umieszczenie "
...
bool Validations::validateZip(QString text) {
  ...
  if (!masks.exactMatch(text)) {
    ...
    QMessageBox::warning(0, "QFaktury",
                         QString("Źle podany kod pocztowy. Wymagany format to "
...
bool Validations::checkSumNIP(QString text) {
  ...
  if (sum == list.last())
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Weryfikator cyfry kontrolnej NIP wykrył niepoprawny numer NIP. "
...
bool Validations::checkSumREGON(QString text) {
  ...
  if (sum == list.last())
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Weryfikator cyfry kontrolnej REGON wykrył niepoprawny numer. "
               "Sprawdź kolejność wpisanych liczb."));
    return false;
  }
}
...
bool Validations::checkSumPESEL(QString text) {
  ...
  if (sum == list.last())
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Weryfikator cyfry kontrolnej PESEL wykrył niepoprawny numer. "
               "Sprawdź kolejność wpisanych liczb."));
    return false;
  }
}
...
bool Validations::checkSumAccount(QString text) {
  ...
  if (symbols().contains(text.at(0)) && symbols().contains(text.at(1))) {
    ...
    if (text.contains(QRegExp("(\\s)").cap()))
      trimmedText = text.remove(" ");
    if (text.contains(QRegExp("(-)").cap()))
      trimmedText = text.remove("-");
    ...
    if (reslong == 1)
      ...
      QMessageBox::warning(
          0, "QFaktury",
          QString("Weryfikator konta bankowego IBAN wykrył niepoprawny numer. "
                 "Sprawdź kolejność wpisanych liczb."));
      return false;
    }
  } else {
...
bool Validations::checkSumIDCard(QString text) {
  ...
  if (sum == result)
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Weryfikator polskiego dowodu osobistego wykrył "
...
bool Validations::checkSumPass(QString text) {
  ...
  if (sum == 0)
    ...
    QMessageBox::warning(
        0, "QFaktury",
        QString("Weryfikator numeru seryjnego paszportu wykrył "
               "niepoprawny numer. Sprawdź kolejność wpisanych "
...
bool Validations::isEmptyField(QLineEdit *field, QString title) {
  ...
  if (field->text().isEmpty()) {
    QMessageBox::warning(0, "QFaktury",
                         QString("Firma nie może zostać zapisana, pownieważ "
