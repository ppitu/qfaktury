...
void DeliveryNote::readWarehouseData(QString invFile) {
  ...
  setWindowTitle(QString("Edytuje WZ"));
...
void DeliveryNote::makeInvoiceSummAll() {
  ...
  invStrList +=
      QString("forma płatności: ") + paysCombo->currentText() + "<br/>";
  invStrList += "</span>";
...
bool DeliveryNote::saveInvoice() {
  ...
  if (this->windowTitle() != s_WIN_WZ_EDIT) {
    ...
    msgBox.setInformativeText(QString("Jeśli chcesz dodać dodatkowo fakturę do "
    ...
    case QMessageBox::Ok: {
      ...
      if (!result) {
        QMessageBox::warning(
            this, QString("Zapis dokumentu WZ"),
            QString("Zapis dokumentu WZ zakończył się niepowodzeniem. Sprawdź, "
                   "czy masz "
                   "uprawnienia do zapisu lub odczytu w ścieżce ") +
