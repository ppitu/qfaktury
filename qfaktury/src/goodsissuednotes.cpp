  ...
  if (!file.open(QIODevice::ReadOnly) || !doc.setContent(stream.readAll())) {
    ...
    if (check_file.exists() && check_file.isFile()) {
      ...
      QMessageBox::warning(
          ...
          QString("Plik przechowujący dane o dokumencie magazynu w ścieżce ") +
...
void GoodsIssuedNotes::makeInvoiceProductsHeadar() {
  ...
  invStrList += "<td align=\"center\" width=\"" + sett().numberToString(res) +
                "%\">" + QString("Nazwa towaru/materiału") + "</td>";

  currentPercent += 9;
  invStrList += "<td align=\"center\" width=\"" + sett().numberToString(9) +
                "%\">" + QString("Ilość żądana") + "</td>";

  currentPercent += 9;
  invStrList += "<td align=\"center\" width=\"" + sett().numberToString(9) +
                "%\">" + QString("Ilość wydana") + "</td>";
...
void GoodsIssuedNotes::makeInvoiceSumm() {
  ...
  invStrList +=
      "<td width=\"21%\" align=\"center\">" + QString("Skąd") + "</td>"; // net
  invStrList +=
      "<td width=\"21%\" align=\"center\">" + QString("Dokąd") + "</td>"; // vat
  invStrList += "<td width=\"31%\" align=\"center\">" +
                QString("Wydział/Dział") + "</td>"; // brutto
...
void GoodsIssuedNotes::makeInvoiceSummAll() {
  ...
  invStrList += "<span>";
  invStrList += QString("Data wydania towaru: ") +
                dateTimeEdit1->date().toString(sett().getDateFormat()) +
                "<br/>";
  invStrList += "</span>";

  invStrList += "<span>";
  invStrList += QString("Data przyjęcia towaru: ") +
...
void GoodsIssuedNotes::makeInvoiceFooter() {
  ...
  invStrList += "<td width=\"3%\">&nbsp;</td>";
  invStrList += "<td width=\"26%\" align=\"center\"> ";
  invStrList += QString("Towar wydał");
  invStrList += "<td width=\"6%\">&nbsp;</td>";
  invStrList += "<td width=\"3%\">&nbsp;</td>";
  invStrList += "<td width=\"26%\" align=\"center\">";
  invStrList += QString("Towar odebrał");
  invStrList += "</td>";
  invStrList += "<td width=\"6%\">&nbsp;</td>";
  invStrList += "<td width=\"3%\">&nbsp;</td>";
  invStrList += "<td width=\"26%\" align=\"center\">";
  invStrList += QString("Zatwierdził");
  invStrList += "</td>";
  invStrList += "</tr>";
  invStrList += "</table>";
  invStrList += "</td></tr>";
...
bool GoodsIssuedNotes::saveInvoice() {
  ...
  if (!result) {
    QMessageBox::warning(
        this, QString("Zapis dokumentu RW"),
        QString("Zapis dokumentu RW zakończył się niepowodzeniem. Sprawdź, czy "
               "masz "
               "uprawnienia do zapisu lub odczytu w ścieżce ") +
            sett().getWarehouseFullDir() +
            QString(" oraz czy ścieżka istnieje."));
  }
