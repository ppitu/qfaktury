...
void GoodsList::doAccept() {
  ...
  if (countSpinBox->text() == "" || countSpinBox->value() < 0.001) {
    QMessageBox::information(this, "QFaktury", QString("Podaj ilość"),
                             QMessageBox::Ok);
  ...
  if (ifGoodIssueNote) {
    if ((amountWant->text() == "" || amountWant->value() < 0.001) &&
      ...
      QMessageBox::information(this, "QFaktury",
                               QString("Podaj ilość żądaną i ilość wydaną"),
                               QMessageBox::Ok);
  ...
  if (selectedItem != "") {
    ...
    QMessageBox::information(this, "QFaktury", QString("Wskaż towar"),
                             QMessageBox::Ok);
  }
}
