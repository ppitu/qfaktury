...
User::User(QWidget *parent) : QDialog(parent) {
  ...
  connect(okButton, &QPushButton::clicked, this, &User::okClick);
    ...
    if (widg->findChild<QComboBox *>("secIdType")->currentText() !=
      ...
      case 1:
        ...
        if (widg->findChild<QLineEdit *>("regonEdit")->text().isEmpty()) {
          QMessageBox::warning(
              this, QString("Pusty NUSP"),
              QString("Pole dla numeru NUSP nie może być puste"));
          return false;
        }
        break;
...
void User::on_nextSeller_clicked() {
  ...
  useLabel->setText(QString("Zastosuj do faktur"));
  vLayout1->addWidget(useLabel);

  QCheckBox *useCheckBox = new QCheckBox(nextSel);
  useCheckBox->setObjectName(QString("useThisSeller"));
  ...
  sellersList->addTab(scrollArea, QString("Sprzedawca/Oddział"));
...
void User::delcurrSel() {
  ...
  if (QMessageBox::warning(this, QString("Usuwanie danych oddziału"),
                           QString("Usunąć dane kolejnego sprzedawcy/oddziału?"),
